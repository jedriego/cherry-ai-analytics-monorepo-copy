...
*   **Unique ID:** `model.google_ads.stg_google_ads__account_stats`
*   **FQN:** `prod.ads_marts.stg_google_ads__account_stats`
...
*   **Columns:**
    ...
    | `AD_NETWORK_TYPE` | 4 | `TEXT` | The Google Ad network type used across the account. | The Google Ad network type used across the account. | `[]` | `{}` |
    | `DEVICE` | 5 | `TEXT` | Account ad performance per unique device where the ads were served. | Account ad performance per unique device where the ads were served. | `[]` | `{}` |
    | `CLICKS` | 6 | `NUMBER` | The number of clicks. | The number of clicks. | `[]` | `{}` |
    | `SPEND` | 7 | `NUMBER` | The sum of your cost-per-click (CPC) and cost-per-thousand impressions (CPM) costs during this period. | The sum of your cost-per-click (CPC) and cost-per-thousand impressions (CPM) costs during this period. | `[]` | `{}` |
    | `IMPRESSIONS` | 8 | `NUMBER` | Count of how often your ad has appeared on a search results page or website on the Google Network. | Count of how often your ad has appeared on a search results page or website on the Google Network. | `[]` | `{}` |
    ...
    | `CONVERSIONS_VALUE` | 10 | `FLOAT` | The sum of monetary values for your `conversions`. You have to enter a value in the Google Ads UI for your conversion actions to make this metric useful. | The sum of monetary values for your `conversions`. You have to enter a value in the Google Ads UI for your conversion actions to make this metric useful. | `[]` | `{}` |
    | `VIEW_THROUGH_CONVERSIONS` | 11 | `NUMBER` | For video campaigns, view-through conversions tell you when an _impression_ of your video ad leads to a conversion on your site. The last impression of a video ad will get credit for the view-through conversion.
...
*   **Depends On (Parents):**
    ...
    *   `model.google_ads.stg_google_ads__account_stats_tmp`
