## Model: `credit_reporting_query_pay_in_four_weekly_xf`

`credit_reporting_query_pay_in_four_weekly_xf`

*   **Unique ID:** `model.cherry_data_model.credit_reporting_query_pay_in_four_weekly_xf`
*   **FQN:** `prod.partners_marts.credit_reporting_query_pay_in_four_weekly_xf`
*   **Description:** **Weekly Credit Bureau Reporting for Pay-in-Four Loans**
This model generates weekly credit bureau reporting files specifically for Cherry's Pay-in-Four  loan products (1.5 month term loans). It implements the same cohort-based reporting system as  the standard weekly model but focuses exclusively on short-term financing products.
**Pay-in-Four Specific Features:** - Includes only loans with 1.5 month original term - Bi-weekly payment frequency ('B' instead of 'M') - Specialized monthly payment calculation for short-term products - Same cohort-based processing as standard weekly model
...
**Key Differences from Standard Weekly Model:** - Filters to include only Pay-in-Four loans (term = 1.5) - Uses bi-weekly terms frequency ('B') - Calculates monthly payment from loan schedule amounts - Processes shorter-term loan lifecycle
**Data Processing:** - Same effective reporting date logic as standard model - Includes both reportable and refunded Pay-in-Four loans - Applies cohort filtering for relevant loans - Uses specialized payment amount calculation
**Use Cases:** - Credit bureau reporting for short-term financing products - Compliance with credit reporting requirements for all loan types - Separate tracking of Pay-in-Four loan performance - Improved credit profile management for short-term borrowers

*   **Tags:** `['external_partners', 'external', 'credit_reporting', 'servicing', 'collections', 'weekly', 'pay_in_four', 'bloom_credit']`
...
*   **Columns:**
 ...
 | `[]` | `{}` |
    | `MONTHLY_PAYMENT` | 29 | `FLOAT` | The scheduled payment amount for Pay-in-Four loans. Calculated from loan schedule  amounts summed for the reporting period, specific to short-term payment structure.
 | The scheduled payment amount for Pay-in-Four loans. Calculated from loan schedule  amounts summed for the reporting period, specific to short-term payment structure.
 | `[]` | `{}` |
 ...
 | `[]` | `{}` |
    | `TERMS_FREQUENCY` | 31 | `TEXT` | The frequency of payments for Pay-in-Four accounts. Set to 'B' for bi-weekly payments  as these are short-term loans with accelerated payment schedules.
 | The frequency of payments for Pay-in-Four accounts. Set to 'B' for bi-weekly payments  as these are short-term loans with accelerated payment schedules.
 | `[]` | `{}` |
    | `TERMS` | 32 | `NUMBER` | The term length of the Pay-in-Four contract. Always 1.5 months (rounded to 2) as  these are short-term financing products.
 | The term length of the Pay-in-Four contract. Always 1.5 months (rounded to 2) as  these are short-term financing products.
 | `[]` | `{}` |
 ...
 | `[]` | `{}` |
    | `DATE_OF_ACCOUNT_INFORMATION` | 35 | `DATE` | The date of the account information being reported. Either the effective reporting  date for open accounts or the later of date_closed and date_of_last_payment for closed accounts. | The date of the account information being reported. Either the effective reporting  date for open accounts or the later of date_closed and date_of_last_payment for closed accounts. | `[]` | `{}` |
*   **Depends On (Parents):**
