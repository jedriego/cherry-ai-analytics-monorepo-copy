## Model: `zendesk_support_conversion_rate_xf`

`zendesk_support_conversion_rate_xf`

*   **Unique ID:** `model.cherry_data_model.zendesk_support_conversion_rate_xf`
*   **FQN:** `prod.customer_support_marts.zendesk_support_conversion_rate_xf`
*   **Description:** This model tracks support ticket interactions that lead to application conversions (applications that are approved or denied). It joins support tickets with borrower details and application status timelines.

*   **Tags:** `['customer_support', 'zendesk_refresh']`
*   **Meta:** `{'sources': [{'name': 'zendesk', 'tables': [{'name': 'tickets', 'description': 'Raw ticket data from Zendesk'}], 'description': 'Source data from Zendesk support system'}, {'name': 'cherry', 'tables': [{'name': 'applications', 'description': 'Raw applications data'}, {'name': 'application_status_timeline', 'description': 'Timeline tracking status changes for applications'}], 'description': 'Application system data source'}], 'monitoring': {'+tags': ['monitoring_metrics'], '+schema': 'monitoring'}}`
*   **Columns:**
    | Name | Index | Type | Comment | Description | Tags | Meta |
    |------|-------|------|---------|-------------|------|------|
    | `TICKET_ID` | 1 | `NUMBER` | Unique identifier for the support ticket | Unique identifier for the support ticket | `[]` | `{}` |
    | `URL` | 2 | `TEXT` |  |  | `[]` | `{}` |
    | `AGENT_NAME` | 3 | `TEXT` |  |  | `[]` | `{}` |
    ...
    | `DIRECTION` | 9 | `TEXT` | Direction of the ticket (filtered to 'Inbound' only) | Direction of the ticket (filtered to 'Inbound' only) | `[]` | `{}` |
    | `IS_TCN_OUTBOUND` | 10 | `BOOLEAN` |  |  | `[]` | `{}` |
    | `BORROWER_ID` | 11 | `TEXT` | Borrower ID associated with the ticket | Borrower ID associated with the ticket | `[]` | `{}` |
    | `MERCHANT_ID` | 12 | `TEXT` |  |  | `[]` | `{}` |
    | `ORGANIZATION_ID` | 13 | `TEXT` |  |  | `[]` | `{}` |
    | `GROUP_NAME` | 14 | `TEXT` |  |  | `[]` | `{}` |
    | `TEAM` | 15 | `TEXT` | Team handling the ticket (filtered to 'Support' only) | Team handling the ticket (filtered to 'Support' only) | `[]` | `{}` |
    | `MANAGER` | 16 | `TEXT` |  |  | `[]` | `{}` |
    | `TICKET_CURRENT_STATUS` | 17 | `TEXT` | Current status of the ticket | Current status of the ticket | `[]` | `{}` |
    | `MEDIA_TYPE_NAME` | 18 | `TEXT` |  |  | `[]` | `{}` |
    | `WAS_CALLBACK` | 19 | `BOOLEAN` |  |  | `[]` | `{}` |
    | `CUSTOMER_TYPE` | 20 | `TEXT` | Type of customer, excluding collections and recovery cases | Type of customer, excluding collections and recovery cases | `[]` | `{}` |
    | `PATIENT_PRACTICE_SEGMENTATION` | 21 | `TEXT` |  |  | `[]` | `{}` |
    | `TIER` | 22 | `TEXT` |  |  | `[]` | `{}` |
    | `TICKET_CREATED_AT_PT` | 23 | `TIMESTAMP_NTZ` | Timestamp when the ticket was created (Pacific Time) | Timestamp when the ticket was created (Pacific Time) | `[]` | `{}` |
    ...
    | `TICKET_RESOLVED_PT` | 27 | `TIMESTAMP_NTZ` | Timestamp when the ticket was resolved (Pacific Time) | Timestamp when the ticket was resolved (Pacific Time) | `[]` | `{}` |
    | `TIME_IN_QUEUE` | 28 | `NUMBER` |  |  | `[]` | `{}` |
    | `WAS_ABANDONED` | 29 | `BOOLEAN` | Flag indicating if the ticket was abandoned | Flag indicating if the ticket was abandoned | `[]` | `{}` |
    ...
    | `AGENT_DISPOSITION` | 58 | `TEXT` | Agent's categorization of the ticket resolution | Agent's categorization of the ticket resolution | `[]` | `{}` |
    ...
    | `APPLICATION_ID` | 190 | `NUMBER` |  |  | `[]` | `{}` |
    | `INITIAL_STATUS` | 191 | `TEXT` | Initial status of the application | Initial status of the application | `[]` | `{}` |
    | `INITIAL_STATUS_PT` | 192 | `TIMESTAMP_NTZ` | Timestamp of the initial application status (Pacific Time) | Timestamp of the initial application status (Pacific Time) | `[]` | `{}` |
    | `STATUS_TWO` | 193 | `TEXT` | Second recorded status of the application | Second recorded status of the application | `[]` | `{}` |
    | `STATUS_TWO_PT` | 194 | `TIMESTAMP_NTZ` | Timestamp of the second status (Pacific Time) | Timestamp of the second status (Pacific Time) | `[]` | `{}` |
    | `STATUS_THREE` | 195 | `TEXT` | Third recorded status of the application | Third recorded status of the application | `[]` | `{}` |
    | `STATUS_THREE_PT` | 196 | `TIMESTAMP_NTZ` | Timestamp of the third status (Pacific Time) | Timestamp of the third status (Pacific Time) | `[]` | `{}` |
    | `STATUS_FOUR` | 197 | `TEXT` | Fourth recorded status of the application | Fourth recorded status of the application | `[]` | `{}` |
    | `STATUS_FOUR_PT` | 198 | `TIMESTAMP_NTZ` | Timestamp of the fourth status (Pacific Time) | Timestamp of the fourth status (Pacific Time) | `[]` | `{}` |
    | `FINAL_STATUS` | 199 | `TEXT` | Final status of the application | Final status of the application | `[]` | `{}` |
    | `FINAL_STATUS_PT` | 200 | `TIMESTAMP_NTZ` | Timestamp of the final status (Pacific Time) | Timestamp of the final status (Pacific Time) | `[]` | `{}` |
    | `INCOME_VERIFICATION_REQUIRED` | 201 | `NUMBER` |  |  | `[]` | `{}` |
    | `KYC_REQUIRED` | 202 | `NUMBER` |  |  | `[]` | `{}` |
    | `FROZEN_AT_SOME_POINT` | 203 | `NUMBER` |  |  | `[]` | `{}` |
    | `TIME_TO_COMPLETION_MINS` | 204 | `NUMBER` | Time to completion in minutes (from initial to final status) | Time to completion in minutes (from initial to final status) | `[]` | `{}` |
    | `TIME_TO_COMPLETION_DAYS` | 205 | `NUMBER` | Time to completion in hours (from initial to final status) | Time to completion in hours (from initial to final status) | `[]` | `{}` |
    | `ATTRIBUTION_WINDOW` | 206 | `NUMBER` | Window in hours used for attribution (fixed at 191 hours - ~7.96 days) | Window in hours used for attribution (fixed at 191 hours - ~7.96 days) | `[]` | `{}` |
    | `CONVERSION_ELIGIBLE` | 207 | `NUMBER` | Flag indicating if the interaction is eligible for conversion attribution (0 or 1) | Flag indicating if the interaction is eligible for conversion attribution (0 or 1) | `[]` | `{}` |
    | `CONVERTED` | 208 | `NUMBER` | Flag indicating if the interaction resulted in a conversion (0 or 1) | Flag indicating if the interaction resulted in a conversion (0 or 1) | `[]` | `{}` |
*   **Depends On (Parents):**
