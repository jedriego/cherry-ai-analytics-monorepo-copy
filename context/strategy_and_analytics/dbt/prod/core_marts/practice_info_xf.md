## Model: `practice_info_xf`

`practice_info_xf`

*   **Unique ID:** `model.cherry_data_model.practice_info_xf`
*   **FQN:** `prod.core_marts.practice_info_xf`
*   **Description:** **Comprehensive Practice Information and Analytics Dimension (Production Clean)**
This model provides a clean, production-ready view of Cherry's complete practice information and analytics dimension, filtered to exclude records with null primary_merchant_id. It serves  as the primary practice dimension table for most analytics, reporting, and operational workflows,  combining base practice data from stg_practices with extensive calculated metrics including early  performance indicators, lifetime value calculations, application success rates, health assessments,  market share data, and retention analytics.
**Key Business Logic:** - **Filtered Data Quality**: Excludes records with null primary_merchant_id to ensure data integrity
  and proper Salesforce account alignment for reliable analytics and reporting
- **Comprehensive Practice Metrics**: Includes all calculated fields from practice_info_full_xf
  such as early performance indicators (first 1, 4, 7, 14, 21, 30, 60 days), lifetime value,
  acquisition costs, health assessments, and retention analytics
...
**Critical Data Sources:** - `practice_info_full_xf`: Parent model containing all base practice data and calculated metrics - `stg_practices`: Core staging model with practice dimension data from Salesforce and merchants - Multiple calculated metric sources: loan totals, application totals, lifetime value, health scores
**Primary Use Cases:** - Primary practice dimension for all production analytics and reporting dashboards - Practice performance monitoring and early success indicator tracking - Retention strategy analysis and health assessment workflows - Sales team performance analysis and practice attribution - Customer lifetime value analysis and ROI calculations
...
Display name of the primary sales opportunity associated with this practice. Used for sales process tracking and opportunity-level analysis.
 | `[]` | `{}` |
    | `PRIMARY_MERCHANT_ID` | 5 | `FLOAT` | **Primary Merchant Identifier (Required Non-Null)**
Primary key for this table representing the unique practice identifier. This field is  guaranteed to be non-null in this filtered model. Similar to cherry_merchant_id but with  special handling for Alle locations which may have 2 distinct merchant_id's - the practice  will be based on the non-Alle merchant_id which is then absorbed as the primary_merchant_id.  Primary merchant ID's maintain a 1-to-1 relationship with Salesforce accounts, which serve  as the source of much of the sales and retention related information in this model.
 | **Primary Merchant Identifier (Required Non-Null)**
Primary key for this table representing the unique practice identifier. This field is  guaranteed to be non-null in this filtered model. Similar to cherry_merchant_id but with  special handling for Alle locations which may have 2 distinct merchant_id's - the practice  will be based on the non-Alle merchant_id which is then absorbed as the primary_merchant_id.  Primary merchant ID's maintain a 1-to-1 relationship with Salesforce accounts, which serve  as the source of much of the sales and retention related information in this model.
 | `[]` | `{}` |
...
Current partnership health score with logic for first 60 days vs standard scoring. Used for ongoing practice health monitoring and retention prioritization.
 | `[]` | `{}` |
    ...
    | `PRACTICE_FIRST_1_APPLICATIONS_COUNT` | 174 | `NUMBER` |  |  | `[]` | `{}` |
    ...
    | `PRACTICE_FIRST_7_APPLICATIONS_COUNT` | 180 | `NUMBER` |  |  | `[]` | `{}` |
...
Total gross loan amount generated in the last 90 days. Part of the recent performance series, used for measuring current practice activity and engagement levels. In USD.
 | `[]` | `{}` |
    ...
    | `ALLE_FIRST_FUNDED_CONTRACT_DATE` | 238 | `TIMESTAMP_NTZ` |  |  | `[]` | `{}` |
    | `CHERRY_FIRST_FUNDED_CONTRACT_DATE` | 239 | `TIMESTAMP_NTZ` |  |  | `[]` | `{}` |
    | `ALLE_LAST_FUNDED_CONTRACT_DATE` | 240 | `TIMESTAMP_NTZ` |  |  | `[]` | `{}` |
    | `CHERRY_LAST_FUNDED_CONTRACT_DATE` | 241 | `TIMESTAMP_NTZ` |  |  | `[]` | `{}` |
    | `PRACTICE_FUNDED_CONTRACTS_COUNT` | 242 | `NUMBER` |  |  | `[]` | `{}` |
...
Monthly cost allocated for retaining this practice, including account management and customer success activities. Used for retention ROI analysis. In USD.
 | `[]` | `{}` |
    | `SALES_DEVELOPMENT_COST` | 254 | `NUMBER` |  |  | `[]` | `{}` |
    | `ACCOUNT_EXECUTIVE_COST` | 255 | `NUMBER` |  |  | `[]` | `{}` |
    | `ONBOARDING_COST` | 256 | `NUMBER` |  |  | `[]` | `{}` |
    | `MARKETING_COST` | 257 | `FLOAT` |  |  | `[]` | `{}` |
...
Calculated practice segment classification. Uses special logic for Alle 'New Practice' types with go-live dates before 2025-06-04 (classified as 'Alle'), otherwise uses the account_industry_segment. Used for strategic segmentation and targeted practice management.
 | `[]` | `{}` |
    ...
    | `INITIAL_HEALTH_ASSESSMENT` | 280 | `TEXT` |  |  | `[]` | `{}` |
    | `INITIAL_HEALTH_ASSESSMENT_NEW_VET_MODEL` | 281 | `TEXT` |  |  | `[]` | `{}` |
    | `ONBOARDING_INITIAL_HEALTH_ASSESSMENT` | 282 | `TEXT` |  |  | `[]` | `{}` |
...
Percentage of practice's total financing volume through Cherry in the past 30 days. Key metric for measuring market share and platform adoption within the practice's total financing activity.
 | `[]` | `{}` |
    | `LAST_PRACTICE_MYSTERY_SHOPPING_DATE` | 285 | `DATE` |  |  | `[]` | `{}` |
    | `LAST_PRACTICE_MYSTERY_SHOPPING_LOOK_RESPONSE` | 286 | `TEXT` |  |  | `[]` | `{}` |
...
Approval rate for applications in the last 30 days. Calculated as approved applications divided by total applications. Key metric for measuring practice application quality and success rates.
 | `[]` | `{}` |
    | `LAST_30_ACTIVE_APPROVAL_RATE` | 289 | `NUMBER` |  |  | `[]` | `{}` |
    | `LAST_30_AVERAGE_APPROVAL_AMOUNT` | 290 | `FLOAT` |  |  | `[]` | `{}` |
    | `OUTSTANDING_APPROVALS` | 291 | `FLOAT` |  |  | `[]` | `{}` |
    | `LAST_30_FUNDING_RATE` | 292 | `NUMBER` |  |  | `[]` | `{}` |
...
Team of the most recent retention owner. Used for retention team performance analysis and workload distribution.
 | `[]` | `{}` |
    | `LATEST_TRUE_RETENTION_OWNER_DEPARTMENT` | 300 | `TEXT` |  |  | `[]` | `{}` |
...
Seasonally adjusted expected monthly Cherry volume that incorporates performance data and parent organization aggregation. Enhanced version of practice potential that considers actual performance patterns. In USD.
 | `[]` | `{}` |
    | `DATE_OF_LATEST_ONSITE_OR_MEANINGFUL_INTERACTION` | 305 | `DATE` |  |  | `[]` | `{}` |
    | `TRAILING_90_ON_SITES` | 306 | `NUMBER` |  |  | `[]` | `{}` |
    | `TRAILING_90_MEANINGFUL_INTERACTIONS` | 307 | `NUMBER` |  |  | `[]` | `{}` |
