## Model: `retention_average_volumes_and_percent_transacting_other_xf`

`retention_average_volumes_and_percent_transacting_other_xf`

*   **Unique ID:** `model.cherry_data_model.retention_average_volumes_and_percent_transacting_other_xf`
*   **FQN:** `prod.revenue_marts.retention_average_volumes_and_percent_transacting_other_xf`
*   **Description:** **"Other" Segment Performance Analytics (Composite Model)**
Specialized composite model for practices that don't fit into core segments (Dental, Aesthetics,  Veterinary), combining volume and transacting metrics from separate component models. This segment  includes smaller practices and non-standard industry classifications.
**Segment Composition:** - "Other" industry segment and select MedSpa practices under Customer Success - Customer Success Plastic Surgery practices not in core segments - Practice potential between $5.25K-$7.5K (mid-tier practices) - Andrew Tallant's specialized "Customer Success Other and Part-Time" assignments
**Composite Architecture:** - Volume metrics from retention_average_volumes_and_percent_transacting_other_volume_xf - Transacting metrics from retention_average_volumes_and_percent_transacting_other_transacting_xf - Different practice potential thresholds for volume ($5.25K+) vs. transacting ($4K+) analysis - Unified reporting format matching other foundational models
...
*   **Depends On (Parents):**
    *   `model.cherry_data_model.retention_average_volumes_and_percent_transacting_other_transacting_xf`
    *   `model.cherry_data_model.retention_average_volumes_and_percent_transacting_other_volume_xf`
