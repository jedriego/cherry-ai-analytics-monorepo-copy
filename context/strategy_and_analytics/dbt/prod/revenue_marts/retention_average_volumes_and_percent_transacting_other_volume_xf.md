## Model: `retention_average_volumes_and_percent_transacting_other_volume_xf`

`retention_average_volumes_and_percent_transacting_other_volume_xf`

*   **Unique ID:** `model.cherry_data_model.retention_average_volumes_and_percent_transacting_other_volume_xf`
*   **FQN:** `prod.revenue_marts.retention_average_volumes_and_percent_transacting_other_volume_xf`
*   **Description:** **"Other" Segment Volume Component Model**
Component model calculating volume-specific metrics for the "Other" segment with higher practice  potential threshold ($5.25K+) to focus volume analysis on practices with meaningful transaction  capacity while maintaining broader engagement metrics separately.
**Filtering Differences:** - Practice potential >$5.25K and ≤$7.5K (volume-focused threshold) - Customer Success Other/Part-Time team plus Andrew Tallant assignments - "Other" and select MedSpa/Plastic Surgery practices - Volume metrics only (average healthy volume, volume targets, trailing averages)
