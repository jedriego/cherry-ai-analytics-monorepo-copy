## Model: `retention_average_volumes_and_percent_transacting_csm_vet_xf`

`retention_average_volumes_and_percent_transacting_csm_vet_xf`

*   **Unique ID:** `model.cherry_data_model.retention_average_volumes_and_percent_transacting_csm_vet_xf`
*   **FQN:** `prod.revenue_marts.retention_average_volumes_and_percent_transacting_csm_vet_xf`
*   **Description:** **Veterinary Segment Performance Analytics**
Specialized model tracking retention performance specifically for Veterinary practices, which  have unique business characteristics and performance patterns requiring dedicated analysis.  Veterinary practices typically have different transaction patterns and growth trajectories.
**Segment Focus:** - Veterinary industry segment exclusively - Cross-departmental (Customer Success, Practice Development, control groups) - All practice sizes included regardless of potential thresholds - Simplified segmentation without practice potential filtering
**Performance Characteristics:** - All transaction volume treated as "healthy" (no At-Risk filtering complexity) - Standard 30-day windows and 7-day trailing averages - Department and team aggregation with Natural Retention Group baseline - Monthly and quarterly target integration from retention targets
